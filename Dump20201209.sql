...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `driver_rating_avg`(In driverNum INT, OUT output FLOAT)
BEGIN

DECLARE avg DEC(10,2) DEFAULT 0.0;
SELECT Campus_Eats_Fall2020.person.person_name, Campus_Eats_Fall2020.ratings.driver_rating
FROM person, orders, ratings
WHERE ratings.order_id = orders.order_id AND orders.person_id = person.person_id AND orders.driver_id = driverNum;
SET avg = output;
...
-- Dump completed on 2020-12-09 11:13:11
